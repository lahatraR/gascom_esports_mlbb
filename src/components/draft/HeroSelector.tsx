    ...
    >
      ...
      <div className="relative">
        <input
          ref={searchRef}
          type="text"
          placeholder="Rechercher un héros… (press /)"
          value={search}
      ...
      {/* ── Hero grid ── */}
      <div className="flex-1 overflow-y-auto min-h-0" style={{ minHeight: 120 }}>
        {heroPool.length === 0 ? (
          ...
          <div className="grid grid-cols-4 sm:grid-cols-5 lg:grid-cols-6 gap-1.5 sm:gap-2 pb-2">
            {filteredHeroes.map((hero) => {
              ...
              return (
                <div
                  key={hero.id}
                  className="relative group/heroitem"
                ...
                >
                  ...
                  {/* What-If simulate button on hover */}
                  {canSimulate && (
                    <button
                      onMouseDown={(e) => { e.preventDefault(); e.stopPropagation(); setPinnedWhatIf(hero); }}
                      className="absolute bottom-0 left-0 right-0 flex items-center justify-center gap-0.5 opacity-0 group-hover/heroitem:opacity-100 transition-opacity duration-150"
                      style={{
                        height: 13, background: 'rgba(250,204,21,0.88)',
                        fontSize: 8, fontWeight: 900, color: '#1a1a00', letterSpacing: '0.05em',
                      }}
      ...
      <div className="flex justify-between text-[10px] text-slate-600">
        <span>
          {availableHeroes.length} available{roleFilter !== 'All' ? ` · ${roleFilter}` : ''}
          {restrictedEnemyIds.length > 0 && (
